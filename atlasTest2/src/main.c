...
void spi_thread(void *arg1, void *arg2, void *arg3)
{
    ...
    dwt_setrxaftertxdelay(ctx, TX_TO_RX_DELAY_UUS);

    /* Set response frame timeout. */
    dwt_setrxtimeout(ctx, RX_RESP_TO_UUS);
    ...
    while (1) {
        ...
        dwt_writetxdata(ctx, sizeof(tx_msg), tx_msg, 0);
        dwt_writetxfctrl(ctx, sizeof(tx_msg), 0, 0);
